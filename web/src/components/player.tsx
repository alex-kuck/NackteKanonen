...
export const PlayerCollapsed = ({player, playerKey, result}: IPlayerProps) => {
    const {
        deposit, fee, poodleFee, bellFee, absentGuest, absentMember, presentGuest, presentMember,
        guestFee, memberFee
    } = result;
...
export const PlayerExpanded = ({player, result, playerKey}: IPlayerProps) => {
    const {
        deposit, fee, poodleFee, bellFee, absentGuest, absentMember, presentGuest, presentMember,
        guestFee, memberFee, poodles, bells
    } = result;
    const totalPresent = presentMember + presentGuest;
    const totalEvents = totalPresent + absentMember + absentGuest;
    const toPay = fee + poodleFee + bellFee + memberFee + guestFee;
